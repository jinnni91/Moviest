...
import com.greedy.moviest.databinding.ActivityWeatherBinding
...
class WeatherActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 오늘 날짜 텍스트뷰 설정
        binding.tvDate.text = SimpleDateFormat("MM월 dd일", Locale.getDefault()).format(Calendar.getInstance().time) + " 오늘의 날씨"
    ...
    private fun setWeather(nx : String, ny : String) {
        ...
        Log.d("날씨정보 확인","${ApiObject.retrofitService}")
        // 비동기적으로 실행하기
        call.enqueue(object : retrofit2.Callback<WEATHER> {
            ...
            override fun onResponse(call: Call<WEATHER>, response: Response<WEATHER>) {
                if (response.isSuccessful) {
                    ...
                    // 리사이클러 뷰에 데이터 연결
                    weatherAdapter?.items =  weatherArr
                    binding.weatherRecyclerView.layoutManager = LinearLayoutManager(this@WeatherActivity, LinearLayoutManager.HORIZONTAL,false)
